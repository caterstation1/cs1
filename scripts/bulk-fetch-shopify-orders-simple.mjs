...
if (!shopUrl || !accessToken || !apiVersion) {
  ...
  console.error('\n💡 Make sure to set these environment variables or run with:')
  console.error('SHOPIFY_SHOP_URL=your-shop.myshopify.com SHOPIFY_ACCESS_TOKEN=your-token SHOPIFY_API_VERSION=2024-01 node scripts/bulk-fetch-shopify-orders-simple.mjs')
  process.exit(1)
}
...
async function fetchAllShopifyOrders() {
  ...
  while (hasNextPage) {
    ...
    const url = nextPageInfo 
      ? `https://${shopUrl}/admin/api/${apiVersion}/orders.json?limit=250&page_info=${nextPageInfo}`
    ...
    try {
      ...
      if (linkHeader && linkHeader.includes('rel="next"')) {
        ...
        if (nextMatch) {
          ...
          console.log(`📄 Found next page, continuing with page_info: ${nextPageInfo}`)
        } else {
