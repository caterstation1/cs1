...
export function ReplyDialog({ isOpen, onClose, parentMessage, onSubmit }: ReplyDialogProps) {
  ...
  // Handle @ autocomplete (same logic as CreateMessageModal)
  ...
  const handleSubmit = async () => {
    ...
    try {
      await onSubmit(content, parentMessage.id)
      setContent('')
      onClose()
    } catch (error) {
          ...
          <DialogDescription>
            {parentMessage && (
              <div className="mt-2 p-3 bg-gray-100 rounded text-sm">
                <div className="font-semibold">{parentMessage.createdByName}</div>
                <div className="text-gray-600 line-clamp-2">{parentMessage.content}</div>
              </div>
            )}
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-4">
          <div className="relative">
            ...
            <Textarea
              ...
              placeholder="Type your reply... Use @ to mention someone or @ORDER-12345 to reference an order."
              rows={4}
