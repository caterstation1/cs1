...
interface Staff {
  ...
  email: string
  payRate: number
  accessLevel: 'wlg_team' | 'wlg_admin'
  isDriver: boolean
  password?: string
}
...
export default function WlgStaffPage() {
  ...
  const fetchStaff = async () => {
    try {
      ...
      // Filter to WLG roles only
      const filtered = (data as any[]).filter(s => s.accessLevel === 'wlg_team' || s.accessLevel === 'wlg_admin')
  ...
  return (
    <div className="container mx-auto py-10">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-bold">WLG Staff</h1>
        <Button onClick={() => setIsAddDialogOpen(true)}>Add WLG Staff</Button>
      </div>
